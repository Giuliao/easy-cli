...
1. Confirm the target host, port, username, and database. easy first loads Home configuration and then project configuration; explicit flags override those values. Do not guess a production or shared database when the target is ambiguous.
2. Ensure the CLI exists. From the repository root, run `make build` when `./easy` is not available.
3. Obtain the password through a secure user-approved channel. A configured password can be used without a flag; when supplying or overriding one, prefer stdin so the secret is not placed directly in shell history:
   ...
   When all connection fields are configured, the command can be simply:
...
- Never run `easy config get mysql.password`: that key is intentionally unavailable.
